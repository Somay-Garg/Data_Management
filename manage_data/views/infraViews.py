...
def save_infra_entry(request,pk):
    ...
    if request.method == "POST":
        ...
        if(request.POST['tieup_university_details']!=''):
            ...
            print(tieup_university_details)
            for key in tieup_university_details:
                print(key)
                if request.method == 'POST' and key in old_tieup_university_details and "tieup_MoU_proof_"+key in request.FILES and request.FILES['tieup_MoU_proof_'+key]:
                    tieup_MoU_proof =request.FILES['tieup_MoU_proof_'+key]
                    fs = FileSystemStorage(location='infra_files/Mou_proof/')
                    filename = fs.save(now.strftime("%H%M%S")+'_'+key+".pdf", tieup_MoU_proof)
                    uploaded_file_url = fs.url(filename) 
                    if(old_tieup_university_details[key]['tieup_MoU_proof']!=''):
                        fs.delete(old_tieup_university_details[key]['tieup_MoU_proof'])
                    tieup_university_details[key]['tieup_MoU_proof'] = uploaded_file_url.split('/')[-1]
                elif "tieup_MoU_proof_"+key in request.FILES and request.FILES['tieup_MoU_proof_'+key]: 
                    tieup_MoU_proof =request.FILES['tieup_MoU_proof_'+key]
                    fs = FileSystemStorage(location='infra_files/Mou_proof/')
                    filename = fs.save(now.strftime("%H%M%S")+'_'+key+".pdf", tieup_MoU_proof)
                    uploaded_file_url = fs.url(filename) 
...
def export_data(request):
    ...
    if 'columns_details' in request.POST:
            ...
            if "id" in req_col:
                req_col.remove('id')
            req_col.remove('proof_UGC_recognition')
            req_col.remove('proof_yr_of_estb_of_college')
            req_col.remove('NBA_accreditation_certificate')
            req_col.remove('NAAC_accreditation_certificate')
            req_col.remove('latest_AQAR_submitted_to_NAAC')
            req_col.remove('list_of_governing_body_members')
            req_col.remove('list_of_academic_advisory_body')
            req_col.remove('list_of_members_in_anti_ragging_committee')
            req_col.remove('list_of_members_in_internal_complaint_committee')
            req_col.remove('list_of_members_in_grievance_redressal_committee')
            req_col.remove('list_of_members_in_disciplane_committee')
            req_col.remove('list_of_members_in_ST_SC_cell')
            req_col.remove('list_of_members_in_disaster_management_cell')
            req_col.remove('list_of_members_in_NSS')
