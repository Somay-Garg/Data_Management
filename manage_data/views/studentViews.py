...
def display_students(request,msg=''):
    ...
    columns = ['id','event_name','event_type','event_date','host_institute','position','team_size','level','date_of_award','upload_proof']
    columns_str = 'id,event_name,event_type,event_date,host_institute,position,team_size,level,date_of_award,upload_proof'
    
    event_data_all = Event.objects.all().values()
    ...
    for event in event_data_all:
        ...
        filter_data['Event Type'].add(event['event_type'])
        filter_data['Host Institute'].add(event['host_institute'])
        filter_data['Position Obtained'].add(event['position'])
        filter_data['Team Size'].add(event['team_size'])
        filter_data['Level'].add(event['level'])
        filter_data['Event Date'].add(event['event_date'])
        filter_data['Award Date'].add(event['date_of_award'])
...
def add_student(request):    
    if request.method == "POST":
        ...
        event.organized_by = organized_by
        event.host_institute = host_institute
        event.position = position
        event.level = level
        event.date_of_award = date_of_award
        event.team_size = team_size
        event.upload_proof = upload_proof
        event.save()
        ...
        if member_details_data != '':
            ...
            for key in member_details_data:
                ...
                student.event_id = last_event.id 
                student.save()
...
def delete_student_entry(request):
    if request.method == "POST":
        ...
        Students.objects.filter(event_id=id).delete()
